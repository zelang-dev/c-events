...
static h2_header_t hpack_predefined[62] = {{NULL, NULL},
												{":authority", NULL},
												{":method", "GET"},
												{":method", "POST"},
												{":path", "/"},
 ...
 * encoded integer. */
static uint64_t hpack_getnum(const uint8_t *buf, int *i, uint8_t idx_mask, http_ini_t *ctx) {
 ...
 * the encoded string. */
static char *hpack_decode(const uint8_t *buf, int *i, int max_i, http_ini_t *ctx) {
	uint64_t byte_len64;
	int byte_len;
	int bit_len;
		...
		for (;;) {
			...
			do {
				...
				if (bitRead > bit_len) {
					...
					return str_dup_ex((char *)str); /* Return a string copy */
				}
...
static void append_bits(uint8_t *target, uint32_t offset, uint32_t value, uint8_t value_bits) {
...
static int hpack_encode(uint8_t *store, string_t load, int lower) {
...
static int is_valid_http2_primer(http_t *conn) {
	size_t pri_len = http2_pri_len;
	char buf[32]; /* Buffer must hold 24 bytes primer */

	int read_pri_len = http_read(conn, buf, pri_len);
	if (read_pri_len != (int)pri_len) {
		/* Size does not match.
		 * This includes cases where http_read returns error codes */
		return 0;
	}
...
static void http2_send_settings(http_t *conn, const struct http2_settings *set) {
	uint16_t id;
	uint32_t data;
	uint8_t http2_settings_frame[9] = {0, 0, 36, 4, 0, 0, 0, 0, 0};
	http_write(conn, http2_settings_frame, 9);

	id = htons(1);
	data = htonl(set->settings_header_table_size);
	http_write(conn, &id, 2);
	http_write(conn, &data, 4);
...
int http2_send_response_headers(http_t *conn) {
	...
	if ((conn->status < 100) || (conn->status > 999)) {
		/* Invalid status: Set status to "Internal Server Error" */
		conn->status = STATUS_INTERNAL_SERVER_ERROR;
	}
	...
	switch (conn->status) {
		...
		default:
			...
			header_bin[header_len++] = 0x30 + (conn->status / 100);
			header_bin[header_len++] = 0x30 + ((conn->status / 10) % 10);
			header_bin[header_len++] = 0x30 + (conn->status % 10);
			break;
	}

	/* Add all headers */
	int counter = 0, count = (int)hash_count(conn->headers), cap = (int)hash_capacity(conn->headers);
	hash_pair_t *pair;
	if (cap > 0 && count > 0) {
		for (i = 0; i < cap; i++) {
			uint16_t predef = 0;
			...
			if (!hash_pair_is_null(pair)) {
				...
				for (j = 15; j <= 61; j++) {
					if (str_is_case(hpack_predefined[j].name, key)) {
						predef = j;
						break;
					}
				}
				...
				if (predef) {
					...
					j = hpack_encode(header_bin + header_len, key, 1);
					header_len += j;
				}

				j = hpack_encode(header_bin + header_len, value, 0);
				header_len += j;

				/* Mark required headers as sent */
				if (str_is_case("Date", key)) {
					has_date = 1;
				}
	...
	/* Send header frame */
	ok = 1;
	if (http_write(conn, http2_header_frame, 9) != 9) {
		ok = 0;
	} else if (http_write(conn, header_bin, header_len) != header_len) {
		ok = 0;
	}
...
void http2_data_frame_head(http_t *conn, uint32_t frame_size, int is_final) {
	...
	http_write(conn, http2_data_frame, 9);
}

static void http2_send_window(http_t *conn, uint32_t stream_id, uint32_t window_size) {
	...
	http_write(conn, http2_window_frame, 9);
	http_write(conn, &data, 4);
}


static void http2_reset_stream(http_t *conn, uint32_t stream_id, uint32_t error_id) {
	...
	http_write(conn, http2_reset_frame, 9);
	http_write(conn, &val, 4);
}
...
static void purge_dynamic_header_table(http_t *conn, uint32_t tableSize) {
	...
	while (conn->http2.dyn_table_size > tableSize) {
		...
		free(conn->http2.dyn_table[conn->http2.dyn_table_size].name);
		conn->http2.dyn_table[conn->http2.dyn_table_size].name = 0;
		free(conn->http2.dyn_table[conn->http2.dyn_table_size].value);
...
static void free_buffered_request_header_list(http_t *conn) {
	while (conn->num_headers > 0) {
		conn->num_headers--;
	}
}

/* Internal function to free header list */
static void free_buffered_response_header_list(http_t *conn) {
	while (conn->num_headers > 0) {
		conn->num_headers--;
	}
}
...
static void handle_http2(http_t *conn) {
	...
	buf = (uint8_t *)malloc(server_settings.settings_max_frame_size);
	if (!buf) {
		/* Out of memory */
	...
	if (is_empty(conn->headers)) {
		conn->headers = hashtable_init(key_ops_auto, val_ops_auto, hash_lp_idx, ARRAY_SIZE);
		defer_free(conn->headers);
	}
	...
	for (;;) {
		...
		bytes_read = http_read(conn, http2_frame_head, sizeof(http2_frame_head));
		...
		bytes_read = http_read(conn, buf, http2_frame_size);
		...
		switch (http2_frame_type) {
			...
			case 1: /* HEADERS */
				{
					...
					while (i < (int)http2_frame_size - (int)padding) {
						...
						if ((buf[i] & 0x80u) == 0x80u) {
							...
							idx_mask = 0x1fu;
							tableSize = hpack_getnum(buf, &i, idx_mask, conn->ctx);
						...
						idx = hpack_getnum(buf, &i, idx_mask, conn->ctx);

						/* Get Header name "key" */
						if (idx == 0) {
							/* Index 0: Header name encoded in following bytes */
							key = hpack_decode(buf, &i, (int)bytes_read, conn->ctx);
					 ...
					 /* Take from dynamic header table */
						...
						if (value_known) {
							...
							if (idx <= 61) {
								if (hpack_predefined[idx].value) {
									val = str_dup_ex(hpack_predefined[idx].value); /* leak? */
								...
								&& ((idx - 61) <= conn->http2.dyn_table_size)) {
								uint32_t local_table_idx = (uint32_t)idx - 62;
								val = str_dup_ex(conn->http2.dyn_table[local_table_idx].value);
								CHECK_LEAK_HDR_ALLOC(val);
							} else {
								/* protocol violation */
								debug_info(
							...
							val = hpack_decode(buf, &i,	(int)bytes_read, conn->ctx); /* leak? */
							CHECK_LEAK_HDR_ALLOC(val);
							if (!val) {
							...
							if (indexing) {
								...
								conn->http2.dyn_table[conn->http2.dyn_table_size]
									.name = str_dup_ex(key); /* leak */
								conn->http2.dyn_table[conn->http2.dyn_table_size]
									.value = str_dup_ex(val); /* leak */
						...
						/* Store these pointers in `hash_http_t` hashtable, will aufofree/clean up later. */
						/* Add header for this request */
						if ((key != NULL) && (val != NULL)
							&& (conn->num_headers < MAX_HEADERS)) {
							hash_put_str(conn->headers, key, (string)val);
							conn->num_headers++;

							/* Some headers need to be stored in the request structure
							 */
							if (!strcmp(":method", key)) {
								snprintf(conn->method, sizeof(conn->method), "%s", val);
							} else if (!strcmp(":path", key)) {
								conn->req.local_uri = val;
							} else if (!strcmp(":status", key)) {
								conn->status = (http_status)atoi(val);
							}
			...
			case 6: /* PING */
				if (http2_frame_flags == 0) {
					...
					http_write(conn, http2_frame_head, sizeof(http2_frame_head));
					http_write(conn, buf, http2_frame_size);
				}
				break;
...
void process_new_http2_connection(http_t *conn) {
	if (!is_valid_http2_primer(conn)) {
		...
		http_error(conn, 400, "%s", "Invalid HTTP/2 primer");
	} else {
