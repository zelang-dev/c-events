 ...
 */
static char *
hpack_decode(const uint8_t *buf, int *i, int max_i, struct http_t *ctx)
{
	uint64_t byte_len64;
	int byte_len;
	int bit_len;
...
static int
hpack_encode(uint8_t *store, string_t load, int lower)
{
...
static int
is_valid_http2_primer(http_t *conn)
{
...
static void
http2_settings_acknowledge(http_t *conn)
{
...
static void
http2_send_settings(http_t *conn,
...
static int
http2_send_response_headers(http_t *conn)
{
...
static void
http2_data_frame_head(http_t *conn,
                      uint32_t frame_size,
                      int is_final)
{
...
static void
http2_reset_stream(http_t *conn,
                   uint32_t stream_id,
                   uint32_t error_id)
{
 ...
 */
static void
free_buffered_request_header_list(http_t *conn)
{
	while (conn->req.num_headers > 0) {
		conn->req.num_headers--;

		CHECK_LEAK_HDR_FREE(
		    conn->req.http_headers[conn->req.num_headers]
		        .name);
		CHECK_LEAK_HDR_FREE(
		    conn->req.http_headers[conn->req.num_headers]
		        .value);

		mg_free((void *)conn->req
		            .http_headers[conn->req.num_headers]
		            .name);
		conn->req.http_headers[conn->req.num_headers].name =
		    0;
		mg_free((void *)conn->req
		            .http_headers[conn->req.num_headers]
		            .value);
		conn->req.http_headers[conn->req.num_headers].value =
		    0;
	}
}
			...
			while (i < (int)http2_frame_size - (int)padding) {
				...
				if (value_known) {
					...
					if (indexing) {
						...
						conn->http2.dyn_table[conn->http2.dyn_table_size].name =
						    mg_strdup_ctx(key, conn->ctx); /* leak */
						conn->http2.dyn_table[conn->http2.dyn_table_size]
						    .value =
						    mg_strdup_ctx(val, conn->ctx); /* leak */
				...
				/* Store these pointers in conn->req[].http_headers,
				...
				if ((key != NULL) && (val != NULL)
					...
					if (!strcmp(":method", key)) {
						conn->req.request_method = val;
					} else if (!strcmp(":path", key)) {
						conn->req.local_uri = val;
						conn->req.request_uri = val;
						conn->req.local_uri_raw = val;
...
static void
process_new_http2_connection(http_t *conn)
{
