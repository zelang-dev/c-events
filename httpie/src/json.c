...
static FORCEINLINE bool isCharAndAdvanceCursor(string_t text, int *cursor, char c) {
...
static FORCEINLINE bool isBoolean(string_t text, int *cursor, int length) {
    return (
...
static FORCEINLINE bool isFraction(string_t text, int *cursor, int length) {
    return text[*cursor] != '.' ||
        (*cursor)++ &&
        isAtLeastOneInteger(text, cursor, length);
}

static FORCEINLINE bool isNumber(string_t text, int *cursor, int length) {
...
static FORCEINLINE bool is_jsonString(string_t text, int *cursor, int length) {
...
FORCEINLINE bool is_string_json(string_t text) {
    int cursor = 0;

    return is_jsonString(text, &cursor, strlen(text));
}

FORCEINLINE bool is_json(json_t *schema) {
