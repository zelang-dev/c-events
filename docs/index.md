...
* Another **safety** feature, don't use standard `malloc` allocation routines. This system *internally* uses a custom [rpmalloc](https://github.com/zelang-dev/rpmalloc), it's *faster*, *atomic* **lock-free**, and actually **signal safe**. This was customizable with `events_set_allocator()`, but [exceptions](https://github.com/zelang-dev/c-events/tree/main/examples/exceptions) **examples** don't run *consistently* correctly with standard **libc**.

> **rpmalloc** usage will detect and display something like **"memory freed: 0, memory used: 5: heap: 0x4ba0170"**, only in **debug builds**, if there is a *memory leak at program exit*. Mixing *rpmalloc (rp_malloc/rp_calloc/rp_realloc/rp_memalign/rpfree)* with *malloc* is **fine** as long as you can *track/match* calls.
