...
thrd_static(ex_memory_t, __scope, null)
thrd_static(ex_context_t, except, null)
...
ex_terminate_func guard_ctrl_c_func = null;
ex_terminate_func guard_terminate_func = null;
...
void ex_trace_set(ex_context_t *ex, void *ctx);
...
ex_ptr_t ex_protect_ptr(ex_ptr_t *const_ptr, void *ptr, void (*func)(void *)) {
    ...
    const_ptr->ptr = ptr;
    ctx->stack = const_ptr;
	const_ptr->type = DATA_EXCEPT_PROTECTED;
    return *const_ptr;
}

void ex_unprotected_ptr(ex_ptr_t *const_ptr) {
	if (!is_except_null() && data_type(const_ptr) == DATA_EXCEPT_PROTECTED) {
        const_ptr->type = DATA_INVALID;
        ex_local()->stack = const_ptr->next;
    }
}

static EVENTS_INLINE bool is_except_root(ex_context_t *ctx) {
#ifdef emulate_tls
    return ctx == ex_local();
#else
	return ctx == &events_except_buffer;
#endif
}
...
static void ex_unwind_stack(ex_context_t *ctx) {
	...
	} else if (ctx->is_unwind && ctx->is_scoped && ctx->data == (void *)scope_local()) {
...
EVENTS_INLINE void ex_unwind_set(ex_context_t *ctx, bool flag) {
    ctx->is_unwind = flag;
}
...
EVENTS_INLINE void ex_swap_reset(ex_context_t *ctx) {
    ctx->data = ctx->prev;
}
...
EVENTS_INLINE ex_context_t *ex_local(void) {
...
int catch_seh(const char *exception, DWORD code, struct _EXCEPTION_POINTERS *ep) {
    ...
    for (i = 0; i < max_ex_sig; i++) {
        if (found || ex_sig[i].seh == code
            ...
            || str_is(ctx->ex, exception)
            ) {
            ctx->state = ex_throw_st;
            ctx->is_rethrown = true;
            if (got_signal || signaled) {
                if (str_is(ctx->ex, exception))
                    ctx->ex = exception;
                else if (!found)
                    ctx->ex = ex_sig[i].ex;
            }
...
EVENTS_INLINE ex_error_t *try_updating_err(ex_error_t *err, ex_context_t *ex_err) {
	...
	if (tasks_is_active())
		task_exception_set((void *)err->name);
	else if (!is___scope_null())
		scope_local()->err = (void *)err->name;

	return err;
}

EVENTS_INLINE void try_rethrow(ex_context_t *ex_err) {
#ifdef _WIN32
...
bool try_catching(char *type, ex_error_t *err, ex_context_t *ex_err) {
    ...
    if (ex_err->state == ex_throw_st) {
        ...
        if (str_is("_if", type)) {
            status = true;
        } else if (is_empty(type) || (str_is(err->name, type))) {
...
EVENTS_INLINE bool try_finallying(ex_error_t *err, ex_context_t *ex_err) {
...
EVENTS_INLINE ex_memory_t *scope_local(void) {
	thrd_local_return(ex_memory_t *, __scope)
}

EVENTS_INLINE void *scope_arena(void) {
	return scope_init()->arena;
}
...
EVENTS_INLINE ex_memory_t *get_scope(void) {
	...
	if (is_empty(scope)) {
		scope = scope_init();
		if (scope->status == DATA_GUARDED_STATUS)
...
void *fence(void *ptr, func_t func) {
	...
	ex_memory_t *scope = get_scope();
	ex_protect_ptr(scope->protector, ptr, func);
	scope->is_protected = true;
	scope_deferred(scope, func, ptr);
...
values_t *trying(void *with, data_func_t tryFunc, data_func_t catchFunc, launch_func_t finalFunc) {
	values_t *value = fence(calloc(1, sizeof(values_t)), events_free);
	if (!is_empty(value)) {
		try {
