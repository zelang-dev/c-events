...
static FORCEINLINE int atomic_cas_ptr(atomic_ptr_t *dst, void *val, void *ref) {
	return (int)atomic_swap(dst, (void *)&ref, val);
}
...
static uint32_t free_list_partial_init(void **list, void **first_block, void *page_start,
	void *block_start, uint32_t block_count, uint32_t block_size) {
...
static void *_rpmalloc_span_initialize_new(heap_t *heap, heap_size_class_t *heap_size_class,
...
static void _rpmalloc_global_cache_insert_spans(span_t **span, size_t span_count, size_t count) {
...
static size_t _rpmalloc_global_cache_extract_spans(span_t **span, size_t span_count, size_t count) {
	global_cache_t *cache = &_memory_span_cache[span_count - 1];
...
static void _rpmalloc_heap_set_reserved_spans(heap_t *heap, span_t *master,
	span_t *reserve, size_t reserve_span_count) {
...
static span_t *_rpmalloc_heap_extract_new_span(heap_t *heap, heap_size_class_t *heap_size_class,
...
static void *_rpmalloc_aligned_reallocate(heap_t *heap, void *ptr, size_t alignment, size_t size, size_t oldsize,
	unsigned int flags) {
	if (alignment <= SMALL_GRANULARITY)
...
extern int rpmalloc_initialize(void) {
...
static FORCEINLINE bool rpmalloc_is_valid(void *ptr) {
	span_t *span = (span_t *)((uintptr_t)ptr & _memory_span_mask);
	if (UNEXPECTED(!span) || ptr == NULL || span->heap == NULL || (intptr_t)span->heap >= LONG_MAX) {
		return false;
	}
