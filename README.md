...
* Another **safety** feature, don't use standard `malloc` allocation routines. This system *internally* uses a custom [rpmalloc](https://github.com/zelang-dev/rpmalloc), it's *faster*, *atomic* **lock-free**, and actually **signal safe**. This is customizable with `events_set_allocator()`, but [exceptions](https://github.com/zelang-dev/c-events/tree/main/examples/exceptions) **examples** don't run *consistently* correctly with standard **libc** `malloc`.
* You can disable **rpmalloc** at *build* `-D NO_RPMALLOC=ON` or `#define NO_RPMALLOC 1`. Then run examples `try_signal_sigsegv.c`, `try_signal_sigfpe.c` or *others* that call `fence()`. The cleanup routine will *intermittently run*, in which, the underlying `struct`, **enum** `type` field not set. The `try/catch` handler was actually catching a **segmentation fault** for the `struct`, before making other changes.

> **rpmalloc** usage will detect and display something like **"freed: 0, used: 5, heap: 0x4ba0170"**, only in **debug builds**, if there is a *memory leak at program exit*. Mixing *rpmalloc (rp_malloc/rp_calloc/rp_realloc/rp_memalign/rpfree)* usage with *malloc* is **fine** as long as you can *track/match* calls, *no crossing*.
...
if(NOT events_FOUND)
    FetchContent_Declare(events
        URL https://github.com/zelang-dev/c-events/archive/refs/tags/0.6.3.zip
        URL_MD5 5eececdf407b2c419a09764afc20b86f
    )
